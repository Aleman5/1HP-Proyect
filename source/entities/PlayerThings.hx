...
class PlayerThings extends FlxSprite 
{
	...
	@:isVar public var followMe(get, set):Bool;
	...
	public function new(?X:Float=0, ?Y:Float=0, _whichPlayer:Int) 
	{
		...
		if(whichPlayer == 1)
			...
			loadGraphic(AssetPaths.player2__png, true, 11, 19);
		scale.set(2, 2);
		updateHitbox();
		width = 6;
		offset.x += 8;
	...
	function get_followMe():Bool 
	{
		return followMe;
	}
	function set_followMe(value:Bool):Bool 
	{
		return followMe = value;
	}
}
